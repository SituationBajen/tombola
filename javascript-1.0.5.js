...
const substitutesresultElem = document.getElementById("substitutesresult");
const substitutesresultlistElem = document.getElementById("substitutesresultlist");
...
codeInputElem.addEventListener("keyup", async (event) => {
  if (event.key === 'Enter') {
    ...
    const substitutes = currentOccasion().substitutes || 0;
    ...
    if (code && quantity) {
      ...
      // Run the tombola:
      const matchTickets = await tombola(currentOccasion().pids, code, quantity, substitutes);
      ...
      if (occasionInputElem.disabled) {
        ...
        resultElem.innerHTML = "<span>" + matchTickets.winners.sort().join("</span> <span>") + "</span>";

        if (matchTickets.substitutes.length > 0) {
            substitutesresultElem.style.display = "block";
            substitutesresultlistElem.innerHTML = "<span>" + matchTickets.substitutes.map((pim, index) => (index + 1) + '. ' + pim).join("</span> <span>") + "</span>";
        }
